...
def detect_entities(row):
    """Uses AWS Comprehend to Create Sentiments on a DataFrame"""

    LOG.info(f"Processing {row}")
    comprehend = boto3.client(service_name='comprehend')
    payload = comprehend.detect_entities(Text=row, LanguageCode='en')
    LOG.debug(f"Found Type: {payload}")
    Type = payload['Entities'][0]['Type']
    return Type
...
def apply_sentiment(df, column="wikipedia_snippit"):
    ...
    df['Type'] = df[column].apply(detect_entities)
    return df
...
def lambda_handler(event, context):
    ...
    # Write result to S3
    write_s3(df=df, bucket="fangsentiment3387", name=names)